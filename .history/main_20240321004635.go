...
func GetVideo(videoUrl string, ctx context.Context) (*fileutils.Video, error) {
	...
	go func() {
		...
		eg.Go(func() error {
			...
			output, err := os.Create(v.Title + ".mp4")
			if err!= nil {
			...
			if err != nil {
				...
				// Close the body regardless of whether the Copy failed
				// or not.
				err = body.Close()
				if err!= nil {
				...
				// Close the response body regardless of whether the Copy failed
				// or not.
				err = resp.Body.Close()
				if err!= nil {
...
func downloading(url string, title string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("GoTube: Failed to create video file: %v", err)
	}
	defer output.Close()

...
func DownloadFile(ctx context.Context, v *fileutils.Video, format *fileutils.Format, outputFile string) error {
	...
	return videoDLWorker(ctx, out, v, format)
}
...
func DownloadComposite(ctx context.Context, outputFile string, v *fileutils.Video, quality string, mimetype, language string) error {
	videoFormat, _, err1 := getVideoAudioFormats(v, quality, mimetype, language)
	if err1 != nil {
		return err1
	}
...
func getVideoWithFormat(videoID string) (*fileutils.Video, *fileutils.Format, error) {
	dl := getDownloader()
...
func downloadVideos(id string) error {
